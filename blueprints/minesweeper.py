...
def create_minesweeper():
    ...
    if 25 > count > 0 and bet < int(user_data['balance']):
        ...
        global user_bet, user_cash_out_val, mines_count, user_guesses_count, mineField
...
def get_field_value():
    ...
    if mineField[pos]:
        ...
        return multiplier, user_cash_out_val


@minesweeper_blueprint.route('/cashOut', methods=["GET"])
def cash_out_minesweeper():
    ...
    played_game(user_id, user_bet, "minesweeper", text_field=user_cash_out_val)

    return "Cashed out ${:.2f}".format(user_cash_out_val)
